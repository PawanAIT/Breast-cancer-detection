...
#print(test_image.size,test_labels.size)
...
with tf.Session() as sess:
  ...
  for step in range(max_steps):
    ...
    if (step % 10) == 0:
      print(step, sess.run(accuracy, feed_dict={x: tempx, y_: tempy, keep_prob: 1.0}))
    sess.run(train_step, feed_dict={x: tempx, y_: tempy, keep_prob: 1.0})
